...
ログイン待機: 案件の `loginRequired` を保存し、利用者の明示的な完了操作まで検索・一括取得・更新確認を無効にする。完了操作はHTMLを取得せず取得ブラウザを閉じる。成功の自動判定はせず、完了状態を永続化しない。通常の手動取得は引き続き利用可能。待機中のUI制御と不明セッション拒否を自動テストする。
