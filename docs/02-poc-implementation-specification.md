...
- ページ上へ `<del>`、`<ins>`、変更ラベルを重ねた赤入れHTMLを保存できる。表の追加・変更箇所は修正後の表内でハイライトし、削除行・削除列は表のDOMへ復元せず、表の直前に位置と削除内容をまとめて表示する。削除内容は非空のセルを先頭2件まで表示し、残りは「ほか○件」と件数で示す
...
| AT-14 | 赤入れHTMLを保存 | 削除・追加・変更位置がページ上で視認でき、表本体は修正後のDOM構造を維持する |
...
- 入れ子表は編集対象外。`rowspan` / `colspan` を含む単純な表は、行・列の追加・削除、行移動、結合・分割、赤入れ表示をサポートする。結合セルをまたぐ列移動および行グループ（`thead` / `tbody` / `tfoot`）をまたぐセル結合は安全のため実行しない
